...
/**
 * 处理微信开放平台扫码登录回调
 * 用户扫描二维码并在微信中确认后，微信开放平台会重定向到此接口
 */
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const tokenUrl = `https://api.weixin.qq.com/sns/oauth2/access_token?appid=${process.env.NEXT_PUBLIC_WECHAT_OPEN_APPID}&secret=${process.env.WECHAT_OPEN_SECRET}&code=${code}&grant_type=authorization_code`
        ...
        console.error('微信扫码登录处理失败:', error)
